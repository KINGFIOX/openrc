...
const struct option longopts[] = {{"directory", 0, NULL, 'd'}, {"directory-truncate", 0, NULL, 'D'},
...
const char *const longopts_help[] = {
    "Create a directory if not exists",          "Create/empty directory", "Create a file if not exists", "Truncate file",
    "Create a named pipe (FIFO) if not exists",  "Mode to check",          "Owner to check (user:group)", "follow symbolic links (irrelevant on linux)",
    "Check whether the path is writable or not", longopts_help_COMMON};
...
static int get_dirfd(char *path, bool symlinks) {
  ...
  if (!path || *path != '/') eerrorx("%s: empty or relative path", applet);
  dirfd = openat(AT_FDCWD, "/", O_RDONLY);
  if (dirfd == -1) eerrorx("%s: unable to open the root directory: %s", applet, strerror(errno));
  ch = path;
  while (*ch) {
    if (*ch == '/') components++;
    ch++;
  ...
  while (dirfd > 0 && item && components > 1) {
    ...
    if (new_dirfd == -1) eerrorx("%s: %s: could not open %s: %s", applet, path, str, strerror(errno));
    if (fstat(new_dirfd, &st) == -1) eerrorx("%s: %s: unable to stat %s: %s", applet, path, item, strerror(errno));
    if (S_ISLNK(st.st_mode)) {
      if (st.st_uid != 0) eerrorx("%s: %s: symbolic link %s not owned by root", applet, path, str);
      linksize = st.st_size + 1;
      if (linkpath) free(linkpath);
      linkpath = xmalloc(linksize);
      memset(linkpath, 0, linksize);
      if (readlinkat(new_dirfd, "", linkpath, linksize) != st.st_size) eerrorx("%s: symbolic link destination changed", applet);
      /*
...
static int do_check(char *path, uid_t uid, gid_t gid, mode_t mode, inode_t type, bool trunc, bool chowner, bool symlinks, bool selinux_on) {
  ...
  if (readfd == -1 || (type == inode_file && trunc)) {
    if (type == inode_file) {
      ...
      if (readfd != -1 && trunc) close(readfd);
      readfd = fd;
  ...
  if (fstat(readfd, &st) != -1) {
    ...
    if (selinux_on) selinux_util_label(path);
  } else {
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) {
    switch (opt) {
      ...
      case 'p':
        ...
        break;
      case 'm':
        if (parse_mode(&mode, optarg) != 0) eerrorx("%s: invalid mode `%s'", applet, optarg);
        break;
      case 'o':
        chowner = true;
  ...
  if (writable && type != inode_unknown) eerrorx("%s: -W cannot be specified along with -d, -f or -p", applet);
  ...
  if (selinux_util_open() == 1) selinux_on = true;

  while (optind < argc) {
    path = clean_path(argv[optind]);
    if (writable) exit(!is_writable(path));
    if (do_check(path, uid, gid, mode, type, trunc, chowner, symlinks, selinux_on)) retval = EXIT_FAILURE;
    optind++;
    free(path);
  }

  if (selinux_on) selinux_util_close();
