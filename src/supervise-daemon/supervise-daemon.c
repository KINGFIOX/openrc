...
const struct option longopts[] = {{"healthcheck-timer", 1, NULL, 'a'},
                                  ...
                                  {"reexec", 0, NULL, '3'},
                                  longopts_COMMON};
const char *const longopts_help[] = {"set an initial health check delay",
...
static void cleanup(void) { free(changeuser); }

RC_NORETURN static void re_exec_supervisor(void) {
  syslog(LOG_WARNING, "Re-executing for %s", svcname);
  execlp("supervise-daemon", "supervise-daemon", svcname, "--reexec", (char *)NULL);
  syslog(LOG_ERR, "Unable to execute supervise-daemon: %s", strerror(errno));
  exit(EXIT_FAILURE);
}
...
static char *make_cmdline(char **argv) {
  ...
  for (c = argv; c && *c; c++) len += (strlen(*c) + 1);
...
static pid_t exec_command(const char *cmd) {
  ...
  if (pid == -1) syslog(LOG_ERR, "fork: %s\n", strerror(errno));
...
RC_NORETURN static void child_process(char *exec, char **argv) {
  ...
  if (nicelevel != INT_MIN) {
    if (setpriority(PRIO_PROCESS, getpid(), nicelevel) == -1) eerrorx("%s: setpriority %d: %s", applet, nicelevel, strerror(errno));
  ...
  if (ionicec != -1 && ioprio_set(1, getpid(), ionicec | ioniced) == -1) eerrorx("%s: ioprio_set %d %d: %s", applet, ionicec, ioniced, strerror(errno));
  ...
  if (redirect_stdout) {
    if ((stdout_fd = open(redirect_stdout, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR)) == -1)
      eerrorx(
  ...
  if (redirect_stderr) {
    if ((stderr_fd = open(redirect_stderr, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR)) == -1)
      eerrorx(
          "%s: unable to open the logfile"
          " for stderr `%s': %s",
  ...
  if (redirect_stdout || stdout_process || rc_yesno(getenv("EINFO_QUIET"))) dup2(stdout_fd, STDOUT_FILENO);
  if (redirect_stderr || stderr_process || rc_yesno(getenv("EINFO_QUIET"))) dup2(stderr_fd, STDERR_FILENO);
...
RC_NORETURN static void supervisor(char *exec, char **argv) {
  ...
  if (svcname) rc_service_daemon_set(svcname, exec, (const char *const *)argv, pidfile, true);
  ...
  while (!exiting) {
    ...
    if (fifo_fd > 0) {
      ...
      close(fifo_fd);
      if (count != -1) buf[count] = 0;
      if (count == 0) continue;
      if (verbose) syslog(LOG_DEBUG, "Received %s from fifo", buf);
      if (strncasecmp(buf, "sig", 3) == 0) {
        if ((sscanf(buf, "%s %d", cmd, &sig_send) == 2) && (sig_send >= 0 && sig_send < NSIG)) {
          syslog(LOG_INFO, "Sending signal %d to %d", sig_send, child_pid);
    ...
    if (do_healthcheck) {
      ...
      if (WIFEXITED(health_status) && WEXITSTATUS(health_status) == 0)
        ...
        nkilled = run_stop_schedule(applet, NULL, NULL, child_pid, 0, false, false, true);
        if (nkilled < 0)
          syslog(LOG_INFO, "Unable to kill %d: %s", child_pid, strerror(errno));
        else
    ...
    if (exiting) {
      ...
      syslog(LOG_INFO, "stopping %s, pid %d", exec, child_pid);
      nkilled = run_stop_schedule(applet, NULL, NULL, child_pid, 0, false, false, true);
      if (nkilled > 0) syslog(LOG_INFO, "killed %d processes", nkilled);
      continue;
    }
    wait_pid = waitpid(child_pid, &i, WNOHANG);
    if (wait_pid == child_pid) {
      if (WIFEXITED(i))
        syslog(LOG_WARNING, "%s, pid %d, exited with return code %d", exec, child_pid, WEXITSTATUS(i));
      else if (WIFSIGNALED(i))
        syslog(LOG_WARNING, "%s, pid %d, terminated by signal %d", exec, child_pid, WTERMSIG(i));
    }
    if (wait_pid == child_pid || healthcheck_respawn) {
      ...
      if (first_spawn == 0) first_spawn = respawn_now;
      if ((respawn_period > 0) && (respawn_now - first_spawn > respawn_period)) {
        respawn_count = 0;
        first_spawn = 0;
      } else
        respawn_count++;
      if (respawn_max > 0 && respawn_count > respawn_max) {
        syslog(LOG_WARNING, "respawned \"%s\" too many times, exiting", exec);
        exiting = 1;
        failing = 1;
        continue;
      }
  ...
  if (svcname) {
    rc_service_daemon_set(svcname, exec, (const char *const *)argv, pidfile, false);
    rc_service_value_set(svcname, "child_pid", NULL);
    rc_service_mark(svcname, RC_SERVICE_STOPPED);
    if (failing) rc_service_mark(svcname, RC_SERVICE_FAILED);
  }
  if (pidfile && exists(pidfile)) unlink(pidfile);
  if (fifopath && exists(fifopath)) unlink(fifopath);
  exit(EXIT_SUCCESS);
}
...
int main(int argc, char **argv) {
  ...
  if (!svcname) eerrorx("%s: The RC_SVCNAME environment variable is not set", applet);
  openlog(applet, LOG_PID, LOG_DAEMON);

  if (argc <= 1 || strcmp(argv[1], svcname)) eerrorx("%s: the first argument is %s and must be %s", applet, argv[1], svcname);
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) switch (opt) {
      case 'a': /* --healthcheck-timer <time> */
        if (sscanf(optarg, "%d", &healthchecktimer) != 1 || healthchecktimer < 1) eerrorx("%s: invalid health check timer %s", applet, optarg);
        break;

      case 'A': /* --healthcheck-delay <time> */
        if (sscanf(optarg, "%d", &healthcheckdelay) != 1 || healthcheckdelay < 1) eerrorx("%s: invalid health check delay %s", applet, optarg);
        break;
      ...
      case 'D': /* --respawn-delay time */
        ...
        if (n != 1 || respawn_delay < 1) eerrorx("Invalid respawn-delay value '%s'", optarg);
        break;
      ...
      case 'P': /* --respawn-period time */
        ...
        if (n != 1 || respawn_period < 1) eerrorx("Invalid respawn-period value '%s'", optarg);
        break;
      ...
      case 'm': /* --respawn-max count */
        ...
        if (n != 1 || respawn_max < 0) eerrorx("Invalid respawn-max value '%s'", optarg);
        break;
  ...
  umask(numask);
  if (!pidfile) xasprintf(&pidfile, "/var/run/supervise-%s.pid", svcname);
  xasprintf(&fifopath, "%s/supervise-%s.ctl", rc_svcdir(), svcname);
  if (mkfifo(fifopath, 0600) == -1 && errno != EEXIST) eerrorx("%s: unable to create control fifo: %s", applet, strerror(errno));
  ...
  if (reexec) {
    ...
    if (pid != -1)
      if (do_stop(applet, exec, (const char *const *)argv, pid, uid, 0, false, true) > 0) eerrorx("%s: %s is already running", applet, exec);

    if (respawn_period > 0 && respawn_delay * respawn_max > respawn_period)
      ewarn(
          "%s: Please increase the value of --respawn-period to more "
    ...
    free(varbuf);
    child_pid = fork();
    if (child_pid == -1) eerrorx("%s: fork: %s", applet, strerror(errno));
    if (child_pid != 0) /* first parent process, do nothing. */
      exit(EXIT_SUCCESS);
#ifdef TIOCNOTTY
    ...
    if (pid != -1) {
      i = kill(pid, SIGTERM);
      if (i != 0) /* We failed to send the signal */
    ...
    if (fifo_fd < 0) eerrorx("%s: unable to open control fifo %s", applet, strerror(errno));
    xasprintf(&str, "sig %d", sig);
    x = write(fifo_fd, str, strlen(str));
    if (x == -1) {
      free(tmp);
      eerrorx("%s: error writing to control fifo: %s", applet, strerror(errno));
    }
    free(tmp);
    exit(EXIT_SUCCESS);
  }
