...
const struct option longopts[] = {{"mount", 0, NULL, 'M'},   {"remount", 0, NULL, 'R'}, {"blockdevice", 0, NULL, 'b'}, {"mountargs", 0, NULL, 'm'},
                                  {"options", 0, NULL, 'o'}, {"passno", 1, NULL, 'p'},  {"fstype", 1, NULL, 't'},      longopts_COMMON};
const char *const longopts_help[] = {"Mounts the filesystem from the mountpoint",
...
static int do_mount(struct ENT *ent, bool remount) {
  ...
  if (err) eerrorx("%s: posix_spawnp: %s", applet, strerror(err));
...
int main(int argc, char **argv) {
  ...
  if (!exists("/etc/fstab")) eerrorx("/etc/fstab does not exist");
    ...
    switch (opt) {
      ...
      case 'p':
        switch (optarg[0]) {
          ...
          case '<':
          case '>':
            if (sscanf(optarg + 1, "%d", &i) != 1) eerrorx("%s: invalid passno %s", argv[0], optarg + 1);

            filtered = true;
            opt = optarg[0];
            START_ENT;
            while ((ent = GET_ENT)) {
              if (strcmp(ENT_FILE(ent), "none") == 0) continue;
              p = ENT_PASS(ent);
              if ((opt == '=' && i == p) || (opt == '<' && i > p && p != 0) || (opt == '>' && i < p && p != 0)) rc_stringlist_add(files, ENT_FILE(ent));
            }
            END_ENT;
            break;
      ...
      case 't':
        ...
        while ((token = strsep(&optarg, ","))) {
          START_ENT;
          while ((ent = GET_ENT))
            if (strcmp(token, ENT_TYPE(ent)) == 0) rc_stringlist_add(files, ENT_FILE(ent));
          END_ENT;
        }
        break;
    ...
    if (TAILQ_FIRST(files)) {
      TAILQ_FOREACH_SAFE(file, files, entries, file_np) {
        for (i = optind; i < argc; i++)
          if (strcmp(argv[i], file->value) == 0) break;
        if (i >= argc) rc_stringlist_delete(files, file->value);
      }
    } else {
      while (optind < argc) rc_stringlist_add(files, argv[optind++]);
    }
  } else if (!filtered) {
    START_ENT;
    while ((ent = GET_ENT)) rc_stringlist_add(files, ENT_FILE(ent));
    END_ENT;

    if (!TAILQ_FIRST(files)) eerrorx("%s: empty fstab", argv[0]);
  }
  ...
  TAILQ_FOREACH(file, files, entries) {
    ...
    if (rc_yesno(getenv("EINFO_QUIET"))) continue;
    ...
    switch (output) {
      ...
      case OUTPUT_MOUNTARGS:
        printf("-o %s -t %s %s %s\n", ENT_OPTS(ent), ENT_TYPE(ent), ENT_BLOCKDEVICE(ent), file->value);
        break;
