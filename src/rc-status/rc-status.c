...
const struct option longopts[] = {
    {"all", 0, NULL, 'a'},      {"crashed", 0, NULL, 'c'},     {"format", 1, NULL, 'f'},     {"list", 0, NULL, 'l'},   {"manual", 0, NULL, 'm'},
    {"runlevel", 0, NULL, 'r'}, {"servicelist", 0, NULL, 's'}, {"supervised", 0, NULL, 'S'}, {"unused", 0, NULL, 'u'}, longopts_COMMON};
const char *const longopts_help[] = {"Show services from all run levels",
...
static void print_level(const char *prefix, const char *level, enum format_t format) {
  switch (format) {
    case FORMAT_DEFAULT:
...
static char *get_uptime(const char *service) {
  ...
  if (state & RC_SERVICE_STARTED) {
    ...
    if (start_count && start_time_string) {
      ...
      if (diff_days > 0)
        xasprintf(&uptime, "%" PRId64 " day(s) %02" PRId64 ":%02" PRId64 ":%02" PRId64 " (%s)", diff_days, diff_hours, diff_mins, diff_secs, start_count);
      else
        xasprintf(&uptime, "%02" PRId64 ":%02" PRId64 ":%02" PRId64 " (%s)", diff_hours, diff_mins, diff_secs, start_count);
    }
    free(start_count);
...
static void print_service(const char *service, enum format_t format) {
  ...
  switch (format) {
    case FORMAT_DEFAULT:
      ...
      if (c && *c && isatty(fileno(stdout))) printf("\n");
...
static void print_services(const char *runlevel, RC_STRINGLIST *svcs, enum format_t format) {
  RC_STRINGLIST *l = NULL;
  RC_STRING *s;
  char *r = NULL;

  if (!svcs) return;
  if (!deptree) deptree = _rc_deptree_load(0, NULL);
  if (!deptree) {
    TAILQ_FOREACH(s, svcs, entries)
    if (!runlevel || rc_service_in_runlevel(s->value, runlevel)) print_service(s->value, format);
    return;
  }
  if (!types) {
...
static void print_stacked_services(const char *runlevel, enum format_t format) {
  ...
  TAILQ_FOREACH(stackedlevel, stackedlevels, entries) {
    if (rc_stringlist_find(levels, stackedlevel->value) != NULL) continue;
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) switch (opt) {
      ...
      case 'm':
        ...
        TAILQ_FOREACH_SAFE(s, services, entries, t)
        if (rc_service_state(s->value) & (RC_SERVICE_STOPPED | RC_SERVICE_HOTPLUGGED)) {
          TAILQ_REMOVE(services, s, entries);
          free(s->value);
          free(s);
        }
      ...
      case 'S':
        services = rc_services_in_state(RC_SERVICE_STARTED);
        TAILQ_FOREACH_SAFE(s, services, entries, t)
        if (!rc_service_value_get(s->value, "child_pid")) TAILQ_REMOVE(services, s, entries);
        print_services(NULL, services, format);
        goto exit;
        /* NOTREACHED */
      case 's':
  ...
  if (!levels) levels = rc_stringlist_new();
  opt = (optind < argc) ? 0 : 1;
  while (optind < argc) {
  ...
  if (show_all || !levels_given) {
    ...
    if (show_all) {
      ...
      if (!runlevel) runlevel = rc_runlevel_get();
      rc_stringlist_add(levels, runlevel);
    }
    ...
    TAILQ_FOREACH_SAFE(s, services, entries, t) {
      ...
      if ((rc_stringlist_find(sservices, s->value) || (state & (RC_SERVICE_STOPPED | RC_SERVICE_HOTPLUGGED)))) {
