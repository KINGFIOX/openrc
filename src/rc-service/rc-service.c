...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) {
    switch (opt) {
      ...
      case 'l':
        ...
        if (TAILQ_FIRST(list) == NULL) return EXIT_FAILURE;
  ...
  if (if_crashed && !(rc_service_daemons_crashed(*argv) && errno != EACCES)) return 0;
  if (if_inactive && !(state & RC_SERVICE_INACTIVE)) return 0;
  if (if_notstarted && (state & RC_SERVICE_STARTED)) return 0;
  if (if_started && !(state & RC_SERVICE_STARTED)) return 0;
  if (if_stopped && !(state & RC_SERVICE_STOPPED)) return 0;
  *argv = service;
  execv(*argv, argv);
