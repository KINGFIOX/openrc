...
static const struct ecolor ecolors[] = {
    {ECOLOR_GOOD, GOOD, "good"},       {ECOLOR_WARN, WARN, "warn"},          {ECOLOR_BAD, BAD, "bad"},
    {ECOLOR_HILITE, HILITE, "hilite"}, {ECOLOR_BRACKET, BRACKET, "bracket"}, {ECOLOR_NORMAL, 0, NULL},
};
...
static const char *const color_terms[] = {
    ...
    "con80x43",     "con80x50",    "con80x60",   "cons25",    "console",      "cygwin",    "dtterm",   "foot",        "gnome",
    "konsole",      "kterm",       "linux",      "linux-c",   "mlterm",       "putty",     "rxvt",     "rxvt-cygwin", "rxvt-cygwin-native",
    "rxvt-unicode", "screen",      "screen-bce", "screen-w",  "screen.linux", "vt100",     "vt220",    "wsvt25",      "xterm",
    "xterm-debian", "xterm-kitty", NULL};
...
static size_t strlcat(char *dst, const char *src, size_t size) {
  ...
  if (src_n == 0) return dst_n + strlen(src);
...
static bool yesno(const char *value) {
  ...
  if (strcasecmp(value, "yes") == 0 || strcasecmp(value, "y") == 0 || strcasecmp(value, "true") == 0 || strcasecmp(value, "on") == 0 ||
      strcasecmp(value, "1") == 0)
    return true;

  if (strcasecmp(value, "no") != 0 && strcasecmp(value, "n") != 0 && strcasecmp(value, "false") != 0 && strcasecmp(value, "off") != 0 &&
...
static bool is_quiet(void) { return yesno(getenv("EINFO_QUIET")); }

static bool is_really_quiet(void) { return yesno(getenv("EERROR_QUIET")); }

static bool is_verbose(void) { return yesno(getenv("EINFO_VERBOSE")); }
...
static bool colour_terminal(FILE *EINFO_RESTRICT f) {
  ...
  if (f && !isatty(fileno(f))) return false;

  if (noyes(getenv("EINFO_COLOR"))) return false;

  if (in_colour == 0) return false;
  if (in_colour == 1) return true;
  ...
  if (strcmp(term, "cons25") == 0) term_is_cons25 = true;

  if (strstr(term, "color")) in_colour = 1;
  ...
  for (i = 0; i < ARRAY_SIZE(ecolors); ++i) {
    ...
    if (ecolors[i].name) {
      ...
      if ((e = getenv("EINFO_COLOR"))) {
        ...
        if (ee) ee += strlen(ecolors[i].name);
...
static int get_term_columns(FILE *EINFO_RESTRICT stream) {
  ...
  if (ioctl(fileno(stream), TIOCGWINSZ, &ws) == 0) return ws.ws_col;

  return DEFAULT_COLS;
}

void eprefix(const char *EINFO_RESTRICT prefix) { _eprefix = prefix; }

static void EINFO_PRINTF(2, 0) elogv(int level, const char *EINFO_RESTRICT fmt, va_list ap) {
...
static int _eindent(FILE *EINFO_RESTRICT stream) {
  ...
  if (env) {
    ...
    if (amount > 0) memset(indent, ' ', (size_t)amount);
  }
...
static const char *_ecolor(FILE *EINFO_RESTRICT f, ECOLOR color) {
  ...
  if (!colour_terminal(f)) return "";

  for (i = 0; i < ARRAY_SIZE(ecolors); ++i)
    if (ecolors[i].color == color) return ecolors_str[i];
  return "";
}
...
static int EINFO_PRINTF(3, 0) _einfo(FILE *f, ECOLOR color, const char *EINFO_RESTRICT fmt, va_list va) {
  int retval = 0;
  char *last = getenv("EINFO_LASTCMD");
  va_list ap;

  if (last && !colour_terminal(f) && strcmp(last, "ewarn") != 0 && last[strlen(last) - 1] == 'n') fprintf(f, "\n");
  if (_eprefix) fprintf(f, "%s%s%s|", _ecolor(f, color), _eprefix, _ecolor(f, ECOLOR_NORMAL));
  fprintf(f, " %s*%s ", _ecolor(f, color), _ecolor(f, ECOLOR_NORMAL));
  retval += _eindent(f);
  va_copy(ap, va);
  retval += vfprintf(f, fmt, ap) + 3;
  va_end(ap);
  if (colour_terminal(f)) fprintf(f, "%s", flush);
  return retval;
}
...
int eerrorn(const char *EINFO_RESTRICT fmt, ...) {
  ...
  if (!fmt || is_really_quiet()) return 0;
  va_start(ap, fmt);
...
int ebegin(const char *EINFO_RESTRICT fmt, ...) {
  ...
  va_end(ap);
  retval += printf(" ...");
  if (colour_terminal(stdout)) retval += printf("\n");
  LASTCMD("ebegin");
  return retval;
}

static void _eend(FILE *EINFO_RESTRICT fp, int col, ECOLOR color, const char *msg) {
  int i;
  int cols;
  ...
  if (term_is_cons25) cols--;

  if (cols > 0 && colour_terminal(fp)) {
    fprintf(fp, "%s%s %s[%s %s %s]%s\n", up, tgoto(goto_column, 0, cols), ecolor(ECOLOR_BRACKET), ecolor(color), msg, ecolor(ECOLOR_BRACKET),
            ecolor(ECOLOR_NORMAL));
  } else {
    if (col > 0)
      for (i = 0; i < cols - col; i++) fprintf(fp, " ");
    fprintf(fp, " [ %s ]\n", msg);
  }
}

static int EINFO_PRINTF(3, 0) _do_eend(const char *cmd, int retval, const char *EINFO_RESTRICT fmt, va_list ap) {
  int col = 0;
  FILE *fp = stdout;
  va_list apc;
  ...
  _eend(fp, col, retval == 0 ? ECOLOR_GOOD : ECOLOR_BAD, retval == 0 ? OK : NOT_OK);
  return retval;
}
...
void ebracket(int col, ECOLOR color, const char *msg) { _eend(stdout, col, color, msg); }
...
void eindent(void) {
  ...
  amount += INDENT_WIDTH;
  if (amount > INDENT_MAX) amount = INDENT_MAX;
...
void eindentv(void) {
  if (is_verbose()) eindent();
}

void eoutdentv(void) {
  if (is_verbose()) eoutdent();
}
