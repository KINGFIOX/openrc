...
const struct option longopts[] = {{"dry-run", 0, NULL, 'd'}, {"omit", 1, NULL, 'o'}, longopts_COMMON};
const char *const longopts_help[] = {"print what would be done", "omit this pid (can be repeated)", longopts_help_COMMON};
...
static int mount_proc(void) {
  ...
  switch (pid) {
    ...
    default:
      ...
      if (rc != pid || WEXITSTATUS(status) != 0) syslog(LOG_ERR, "mount returned non-zero exit status");
      break;
  }
...
static int signal_processes(int sig, RC_STRINGLIST *omits, bool dryrun) {
  ...
  while ((d = readdir(dir)) != NULL) {
    ...
    xasprintf(&buf, "%d", pid);
    if (rc_stringlist_find(omits, buf)) continue;

    /* Is this process in our session? */
    if (getsid(getpid()) == getsid(pid)) continue;
