...
void rc_plugin_load(void) {
  ...
  if (rc_in_plugin) return;

  TAILQ_INIT(&plugins);

  if (!(dp = opendir(RC_PLUGINDIR))) return;
...
void rc_plugin_run(RC_HOOK hook, const char *value) {
  ...
  TAILQ_FOREACH(plugin, &plugins, entries) {
    ...
    for (i = 0; i < 2; i++)
      if ((flags = fcntl(pfd[i], F_GETFD, 0)) < 0 || fcntl(pfd[i], F_SETFD, flags | FD_CLOEXEC) < 0) eerror("fcntl: %s", strerror(errno));
