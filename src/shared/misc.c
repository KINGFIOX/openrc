...
static const char *const env_whitelist[] = {"EERROR_QUIET", "EINFO_QUIET", "IN_BACKGROUND", "IN_DRYRUN",   "IN_HOTPLUG",    "RC_DEBUG", "RC_NODEPS",
                                            "LANG",         "LC_MESSAGES", "TERM",          "EINFO_COLOR", "EINFO_VERBOSE", NULL};
...
void env_filter(void) {
  ...
  TAILQ_FOREACH(env, env_list, entries) {
    ...
    for (i = 0; env_whitelist[i]; i++) {
      if (strcmp(env_whitelist[i], env->value) == 0) break;
    }
    if (env_whitelist[i]) continue;

    /* Check our user defined list */
    if (rc_stringlist_find(env_allow, env->value)) continue;
...
void env_config(void) {
  ...
  if (!path)
    ...
    while ((token = strsep(&path, ":"))) {
      ...
      while ((tok = strsep(&npp, ":")))
        if (strcmp(tok, token) == 0) break;
      if (!tok) p += snprintf(p, l - (p - e), ":%s", token);
      free(np);
    }
    *p++ = '\0';
    unsetenv("PATH");
  ...
  if ((fp = fopen(RC_KRUNLEVEL, "r"))) {
    if (xgetline(&buffer, &size, fp) != -1) setenv("RC_DEFAULTLEVEL", buffer, 1);
    free(buffer);
    fclose(fp);
  } else {
  ...
  if (rc_conf_yesno("rc_quiet")) setenv("EINFO_QUIET", "YES", 1);
  if (rc_conf_yesno("rc_verbose")) setenv("EINFO_VERBOSE", "YES", 1);

  errno = 0;
  if ((!rc_conf_yesno("rc_color") && errno == 0) || rc_conf_yesno("rc_nocolor")) setenv("EINFO_COLOR", "NO", 1);
}
...
int svc_lock(const char *applet, bool ignore_lock_failure) {
  char *file = NULL;
  int fd;
...
RC_DEPTREE *_rc_deptree_load(int force, int *regen) {
  ...
  if (rc_deptree_update_needed(&t, file) || force != 0) {
    ...
    merrno = errno;
    errno = serrno;
    if (fd == -1 && merrno == EACCES) goto out;
    close(fd);
    ...
    if (retval == 0) {
      ...
      if (st.st_mtime < t) {
        ...
        if (exists(deptree_skewed)) unlink(deptree_skewed);
      }
      free(deptree_skewed);
    }
    if (force == -1 && regen != NULL) *regen = retval;
  out:
    free(deptree_cache);
  }
...
RC_SERVICE lookup_service_state(const char *service) {
  ...
  for (i = 0; i < ARRAY_SIZE(service_bits); ++i)
    if (!strcmp(service, service_bits[i].name)) return service_bits[i].bit;
  return 0;
}

void from_time_t(char *time_string, time_t tv) { strftime(time_string, 20, "%Y-%m-%d %H:%M:%S", localtime(&tv)); }
...
time_t to_time_t(char *timestring) {
  ...
  check = sscanf(timestring, "%4d-%2d-%2d %2d:%2d:%2d", &year, &month, &day, &hour, &min, &sec);
  if (check == 6) {
...
static inline int close_range(int first RC_UNUSED, int last RC_UNUSED, unsigned int flags RC_UNUSED) {
...
void cloexec_fds_from(int first) {
  int i;
  if (close_range(first, UINT_MAX, CLOSE_RANGE_CLOEXEC) < 0) {
    for (i = getdtablesize() - 1; i >= first; --i) fcntl(i, F_SETFD, FD_CLOEXEC);
  }
}
