...
#define signalpair_item(name) {#name, SIG##name},
  ...
  static const SIGNALPAIR signallist[] = {
      signalpair_item(HUP) signalpair_item(INT) signalpair_item(QUIT) signalpair_item(ILL) signalpair_item(TRAP) signalpair_item(ABRT) signalpair_item(BUS)
          signalpair_item(FPE) signalpair_item(KILL) signalpair_item(USR1) signalpair_item(SEGV) signalpair_item(USR2) signalpair_item(PIPE)
              signalpair_item(ALRM) signalpair_item(TERM) signalpair_item(CHLD) signalpair_item(CONT) signalpair_item(STOP) signalpair_item(TSTP)
                  signalpair_item(TTIN) signalpair_item(TTOU) signalpair_item(URG) signalpair_item(XCPU) signalpair_item(XFSZ) signalpair_item(VTALRM)
  ...
  for (i = 0; i < ARRAY_SIZE(signallist); ++i)
    if (strcmp(sig, signallist[i].name) == 0 || (s && strcmp(s, signallist[i].name) == 0)) return signallist[i].signal;
...
static SCHEDULEITEM *parse_schedule_item(const char *applet, const char *string) {
  ...
  if (strcmp(string, "forever") == 0)
    ...
    errno = 0;
    if (sscanf(string, "%d", &item->value) != 1) eerrorx("%s: invalid timeout value in schedule `%s'", applet, string);
  } else if ((after_hyph = string + (string[0] == '-')) && ((sig = parse_signal(applet, after_hyph)) != -1)) {
    item->type = SC_SIGNAL;
    item->value = (int)sig;
  } else
...
void parse_schedule(const char *applet, const char *string, int timeout) {
  ...
  if (count == 0) {
    ...
    TAILQ_INSERT_TAIL(&schedule, item, entries);
    if (string) {
      if (sscanf(string, "%d", &item->value) != 1) eerrorx("%s: invalid timeout in schedule", applet);
    } else
      item->value = 5;
  ...
  while (string != NULL) {
    ...
    if (len >= (ptrdiff_t)sizeof(buffer)) eerrorx("%s: invalid schedule item, far too long", applet);
...
int do_stop(const char *applet, const char *exec, const char *const *argv, pid_t pid, uid_t uid, int sig, bool test, bool quiet) {
  RC_PIDLIST *pids;
  RC_PID *pi;
  RC_PID *np;
  bool killed;
  ...
  LIST_FOREACH_SAFE(pi, pids, entries, np) {
    if (test) {
      ...
      if (sig) {
        ...
        if (!quiet) ebeginv("Sending signal %d to PID %d", sig, pi->pid);
      }
      errno = 0;
      killed = (kill(pi->pid, sig) == 0 || errno == ESRCH ? true : false);
      if (!quiet)
        eendv(killed ? 0 : 1, "%s: failed to send signal %d to PID %d: %s", applet, sig, pi->pid, strerror(errno));
      else if (!killed)
        syslog(LOG_ERR, "Failed to send signal %d to PID %d: %s", sig, pi->pid, strerror(errno));
      if (!killed) {
        nkilled = -1;
      } else {
        if (nkilled != -1) nkilled++;
      }
    }
    free(pi);
  }
...
int run_stop_schedule(const char *applet, const char *exec, const char *const *argv, pid_t pid, uid_t uid, bool test, bool progress, bool quiet) {
  ...
  if (!(pid > 0 || exec || uid || (argv && *argv))) return 0;
  ...
  while (item) {
    switch (item->type) {
      ...
      case SC_SIGNAL:
        nrunning = 0;
        nkilled = do_stop(applet, exec, argv, pid, uid, item->value, test, quiet);
        if (nkilled == 0) {
          if (tkilled == 0) {
            if (progressed) printf("\n");
      ...
      case SC_TIMEOUT:
        ...
        for (nsecs = 0; item->type == SC_FOREVER || nsecs < item->value; nsecs++) {
          for (nloops = 0; nloops < ONE_SECOND / POLL_INTERVAL; nloops++) {
    ...
    if (item) item = TAILQ_NEXT(item, entries);
  }

  if (test || (tkilled > 0 && nrunning == 0)) return nkilled;
