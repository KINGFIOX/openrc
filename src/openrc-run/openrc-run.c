...
const char *const longopts_help[] = {
    "set xtrace when running the script", "show what would be done", "only run commands when started", "only run commands when stopped", "ignore dependencies",
    "fd of the exclusive lock from rc",   longopts_help_COMMON};
...
static void handle_signal(int sig) {
  ...
  switch (sig) {
    ...
    case SIGCHLD:
      while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        if (signal_pipe[1] > -1 && pid == service_pid) {
          if (write(signal_pipe[1], &status, sizeof(status)) == -1) eerror("%s: send: %s", service, strerror(errno));
        }
      }
      break;
    ...
    case SIGQUIT:
      ...
      if (service_pid > 0) kill(service_pid, sig);
...
static void unhotplug(void) {
  char *file = NULL;

  xasprintf(&file, "%s/hotplugged/%s", rc_svcdir(), applet);
  if (exists(file) && unlink(file) != 0) eerror("%s: unlink `%s': %s", applet, file, strerror(errno));
  free(file);
}
...
static void start_services(RC_STRINGLIST *list) {
  ...
  if (state & RC_SERVICE_INACTIVE || state & RC_SERVICE_WASINACTIVE || state & RC_SERVICE_STARTING || state & RC_SERVICE_STARTED) {
    TAILQ_FOREACH(svc, list, entries) {
      if (!(rc_service_state(svc->value) & RC_SERVICE_STOPPED)) continue;
...
static void restore_state(void) {
  ...
  if (rc_in_plugin || exclusive_fd == -1) return;
  ...
  if (state & RC_SERVICE_STOPPING) {
    ...
    if (rc_runlevel_stopping()) rc_service_mark(applet, RC_SERVICE_FAILED);
  } else if (state & RC_SERVICE_STARTING) {
    if (state & RC_SERVICE_WASINACTIVE)
      rc_service_mark(applet, RC_SERVICE_INACTIVE);
    else
      rc_service_mark(applet, RC_SERVICE_STOPPED);
    if (rc_runlevel_starting()) rc_service_mark(applet, RC_SERVICE_FAILED);
  }
...
static void cleanup(void) {
  ...
  if (!rc_in_plugin) {
    ...
    if (restart_services) start_services(restart_services);
  }
...
static int write_prefix(const char *buffer, size_t bytes, bool *prefixed) {
  ...
  for (i = 0; i < bytes; i++) {
    ...
    if (buffer[i] == '\033' && !*prefixed) {
      for (j = i + 1; j < bytes; j++) {
        if (buffer[j] == 'A') *prefixed = true;
        if (isalpha((unsigned int)buffer[j])) break;
      }
    }
...
static int openrc_sh_exec(const char *openrc_sh, const char *arg1, const char *arg2) {
  if (arg2)
...
static int svc_exec(const char *arg1, const char *arg2) {
  ...
  if (pipe(signal_pipe) == -1) eerrorx("%s: pipe: %s", service, applet);
  for (i = 0; i < 2; i++)
    if ((flags = fcntl(signal_pipe[i], F_GETFD, 0) == -1 || fcntl(signal_pipe[i], F_SETFD, flags | FD_CLOEXEC) == -1))
  ...
  if (prefix && isatty(fdout)) {
    ...
    if (master_tty >= 0 && (flags = fcntl(master_tty, F_GETFD, 0)) == 0) fcntl(master_tty, F_SETFD, flags | FD_CLOEXEC);

    if (slave_tty >= 0 && (flags = fcntl(slave_tty, F_GETFD, 0)) == 0) fcntl(slave_tty, F_SETFD, flags | FD_CLOEXEC);
  }

  service_pid = fork();
  if (service_pid == -1) eerrorx("%s: fork: %s", service, strerror(errno));
  if (service_pid == 0) {
    char *openrc_sh;
  ...
  for (;;) {
    if ((s = poll(fd, master_tty >= 0 ? 2 : 1, -1)) == -1) {
      if (errno != EINTR) {
        eerror("%s: poll: %s", service, strerror(errno));
        ret = -1;
        break;
      }
    }
    ...
    if (s > 0) {
      ...
      if (fd[0].revents & (POLLIN | POLLHUP)) {
        if ((s = read(signal_pipe[0], &ret, sizeof(ret))) != sizeof(ret)) {
          eerror("%s: receive failed: %s", service, s < 0 ? strerror(errno) : "short read");
          ret = -1;
          break;
        }
        ret = WEXITSTATUS(ret);
        if (ret != 0 && errno == ECHILD) /* killall5 -9 could cause this */
          ret = 0;
        break;
      }
    }
...
static bool svc_wait(const char *svc) {
  ...
  if (rc_stringlist_find(keywords, "-timeout") || rc_stringlist_find(keywords, "notimeout")) forever = true;
  ...
  for (;;) {
    ...
    if (errno != EWOULDBLOCK) {
      eerror("%s: open `%s': %s", applet, file, strerror(errno));
      free(file);
      exit(EXIT_FAILURE);
    }
    if (nanosleep(&interval, NULL) == -1) {
      if (errno != EINTR) goto finish;
    }
    if (!forever) {
      timespecsub(&timeout, &interval, &timeout);
      if (timeout.tv_sec <= 0) goto finish;
      timespecsub(&warn, &interval, &warn);
      if (warn.tv_sec <= 0) {
        ewarn("%s: waiting for %s (%d seconds)", applet, svc, (int)timeout.tv_sec);
        warn.tv_sec = WARN_TIMEOUT;
        warn.tv_nsec = 0;
      }
    }
...
static void svc_start_check(void) {
  ...
  if (in_background) {
    if (!(state & (RC_SERVICE_INACTIVE | RC_SERVICE_STOPPED))) exit(EXIT_FAILURE);
    if (rc_yesno(getenv("IN_HOTPLUG"))) rc_service_mark(service, RC_SERVICE_HOTPLUGGED);
    if (strcmp(runlevel, RC_LEVEL_SYSINIT) == 0)
      ewarnx(
          "WARNING: %s will be started in the"
          " next runlevel",
          applet);
  }

  if (exclusive_fd == -1) exclusive_fd = svc_lock(applet, !deps);
  if (exclusive_fd == -1) {
    if (errno == EACCES) eerrorx("%s: superuser access required", applet);
    if (state & RC_SERVICE_STOPPING)
...
static void svc_start_deps(void) {
  ...
  errno = 0;
  if (rc_conf_yesno("rc_depend_strict") || errno == ENOENT) depoptions |= RC_DEP_STRICT;

  if (!deptree && ((deptree = _rc_deptree_load(0, NULL)) == NULL)) eerrorx("failed to load deptree");
  if (!deptypes_b) setup_deptypes();

  services = rc_deptree_depends(deptree, deptypes_b, applet_list, runlevel, 0);
  ...
  if (!rc_runlevel_starting()) {
    TAILQ_FOREACH(svc, use_services, entries) {
      ...
      if (state & RC_SERVICE_FAILED && rc_runlevel_starting()) continue;
      if (state & RC_SERVICE_STOPPED) {
        ...
        if (!rc_conf_yesno("rc_parallel")) rc_waitpid(pid);
      }
    }
  }
  ...
  TAILQ_FOREACH(svc, services, entries) {
    state = rc_service_state(svc->value);
    if (state & RC_SERVICE_STARTED) continue;
    ...
    if (state & RC_SERVICE_STARTING && state & RC_SERVICE_WASINACTIVE) {
      if (!rc_stringlist_find(need_services, svc->value) && !rc_stringlist_find(want_services, svc->value) && !rc_stringlist_find(use_services, svc->value))
        continue;
    }

    if (!svc_wait(svc->value)) eerror("%s: timed out waiting for %s", applet, svc->value);
    state = rc_service_state(svc->value);
    if (state & RC_SERVICE_STARTED) continue;
...
static void svc_start_real(void) {
  ...
  if (!skip_mark) rc_service_mark(service, RC_SERVICE_STARTED);
  exclusive_fd = svc_unlock(applet, exclusive_fd);
  hook_out = RC_HOOK_SERVICE_START_OUT;
  ...
  TAILQ_FOREACH(svc, services, entries)
  if (rc_service_state(svc->value) & RC_SERVICE_STOPPED) service_start(svc->value);
  rc_stringlist_free(services);
  services = NULL;
  ...
  if (deptree) {
    ...
    TAILQ_FOREACH(svc, tmplist, entries) {
      ...
      if (rc_service_state(svc2->value) & RC_SERVICE_STOPPED) service_start(svc2->value);
      rc_stringlist_free(services);
      services = NULL;
    }
    rc_stringlist_free(tmplist);
...
static int svc_stop_check(RC_SERVICE *state) {
  *state = rc_service_state(service);
  ...
  if (rc_runlevel_stopping() && *state & RC_SERVICE_FAILED) exit(EXIT_FAILURE);

  if (in_background && !(*state & RC_SERVICE_STARTED) && !(*state & RC_SERVICE_INACTIVE)) exit(EXIT_FAILURE);

  if (exclusive_fd == -1) exclusive_fd = svc_lock(applet, !deps);
  if (exclusive_fd == -1) {
    if (errno == EACCES) eerrorx("%s: superuser access required", applet);
    if (*state & RC_SERVICE_STOPPING) ewarnx("WARNING: %s is already stopping", applet);
...
static void svc_stop_deps(RC_SERVICE state) {
  ...
  if (state & RC_SERVICE_WASINACTIVE) return;
  ...
  TAILQ_FOREACH_REVERSE(svc, services, rc_stringlist, entries) {
     ...
     * If you remove this check, ensure that the
     * exclusive file isn't created. */
    if (state & RC_SERVICE_FAILED && rc_runlevel_stopping()) continue;
  ...
  TAILQ_FOREACH(svc, tmplist, entries) {
    ...
    if (rc_runlevel_stopping()) {
      ...
      if (runlevel && (strcmp(runlevel, RC_LEVEL_SHUTDOWN) == 0 || strcmp(runlevel, RC_LEVEL_SINGLE) == 0)) continue;
      rc_service_mark(service, RC_SERVICE_FAILED);
    }
    eerrorx(
        "ERROR: cannot stop %s as %s "
        "is still up",
        applet, svc->value);
  }
  rc_stringlist_free(tmplist);
...
static void svc_stop_real(void) {
  ...
  if (strcmp(applet, "localmount") == 0) setenv("LC_ALL", "C", 1);
  ...
  if (!stopped) eerrorx("ERROR: %s failed to stop", applet);
...
static void svc_restart(void) {
  if (!(rc_service_state(service) & RC_SERVICE_STOPPED)) {
    ...
    if (dry_run)
      ewarn(
          "Cannot calculate restart start dependencies"
          " on a dry-run");
  }
...
static bool service_plugable(void) {
  ...
  if (!match) match = rc_conf_value("rc_plug_services");
...
int main(int argc, char **argv) {
  ...
  free(lnk);
  if (!service) service = xstrdup(path);
  applet = basename_c(service);

  if (argc < 3) usage(EXIT_FAILURE);

  /* Change dir to / to ensure all init scripts don't use stuff in pwd */
  if (chdir("/") == -1) eerror("chdir: %s", strerror(errno));
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) switch (opt) {
      ...
      case 'l':
        ...
        break;
      case 's':
        if (!(rc_service_state(service) & RC_SERVICE_STARTED)) exit(EXIT_FAILURE);
        break;
      case 'S':
        if (!(rc_service_state(service) & RC_SERVICE_STOPPED)) exit(EXIT_FAILURE);
        break;
      case 'D':
        deps = false;
        break;
  ...
  if (rc_yesno(getenv("RC_NODEPS"))) deps = false;

  /* If we're changing runlevels and not called by rc then we cannot
     work with any dependencies */
  if (deps && getenv("RC_PID") == NULL && (rc_runlevel_starting() || rc_runlevel_stopping())) deps = false;
  ...
  if (rc_yesno(getenv("IN_DRYRUN"))) dry_run = true;
  if (rc_yesno(getenv("IN_HOTPLUG"))) {
    if (!service_plugable()) eerrorx("%s: not allowed to be hotplugged", applet);
    in_background = true;
  }
  ...
  while (optind < argc) {
    ...
    if (strcmp(optarg, "describe") == 0 || strcmp(optarg, "help") == 0 || strcmp(optarg, "depend") == 0) {
      save = prefix;
      eprefix(NULL);
      prefix = NULL;
      ...
      eprefix(save);
      prefix = save;
    } else if (strcmp(optarg, "ineed") == 0 || strcmp(optarg, "iuse") == 0 || strcmp(optarg, "iwant") == 0 || strcmp(optarg, "needsme") == 0 ||
               strcmp(optarg, "usesme") == 0 || strcmp(optarg, "wantsme") == 0 || strcmp(optarg, "iafter") == 0 || strcmp(optarg, "ibefore") == 0 ||
      ...
      if (strcmp(optarg, "conditionalrestart") == 0 || strcmp(optarg, "condrestart") == 0) {
        if (rc_service_state(service) & RC_SERVICE_STARTED) svc_restart();
        ...
        if (deps && in_background) get_started_services();
        if (svc_stop() == 1) continue; /* Service has been stopped already */
        if (deps) {
          if (!in_background && !rc_runlevel_stopping() && rc_service_state(service) & RC_SERVICE_STOPPED) unhotplug();

          if (in_background && rc_service_state(service) & RC_SERVICE_INACTIVE) {
            TAILQ_FOREACH(svc, restart_services, entries)
            if (rc_service_state(svc->value) & RC_SERVICE_STOPPED) rc_service_schedule_start(service, svc->value);
          }
        }
      } else if (strcmp(optarg, "zap") == 0) {
        einfo("Manually resetting %s to stopped state", applet);
        if (!rc_service_mark(applet, RC_SERVICE_STOPPED)) eerrorx("rc_service_mark: %s", strerror(errno));
        unhotplug();
      } else
