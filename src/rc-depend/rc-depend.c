...
const struct option longopts[] = {{"starting", 0, NULL, 'a'}, {"stopping", 0, NULL, 'o'}, {"type", 1, NULL, 't'},         {"notrace", 0, NULL, 'T'},
                                  {"strict", 0, NULL, 's'},   {"update", 0, NULL, 'u'},   {"deptree-file", 1, NULL, 'F'}, longopts_COMMON};
const char *const longopts_help[] = {"Order services as if runlevel is starting",
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) {
    switch (opt) {
      ...
      case 's':
        ...
        break;
      case 't':
        while ((token = strsep(&optarg, ","))) rc_stringlist_add(types, token);
        break;
      case 'u':
        update = 1;
        break;
  ...
  if (deptree_file) {
    if (!(deptree = rc_deptree_load_file(deptree_file))) eerrorx("failed to load deptree");
  } else {
    if (!(deptree = _rc_deptree_load(update, NULL))) eerrorx("failed to load deptree");
  }
