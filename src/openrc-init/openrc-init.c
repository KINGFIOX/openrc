...
static void init(const char *default_runlevel) {
  ...
  if (!runlevel) runlevel = rc_default_runlevel;
...
static void run_program(const char *prog) {
  ...
  sigprocmask(SIG_SETMASK, &old, NULL);
  if (rc_waitpid(pid) == -1) perror("init");
}
...
static void handle_single(void) { do_openrc("single"); }
...
int main(int argc, char **argv) {
  ...
  if (default_runlevel && strcmp(default_runlevel, "reexec") == 0) reexec = true;
  ...
  if (!reexec) init(default_runlevel);

  if (mkfifo(RC_INIT_FIFO, 0600) == -1 && errno != EEXIST) perror("mkfifo");

  for (;;) {
    /* This will block until a command is sent down the pipe... */
    fifo = fopen(RC_INIT_FIFO, "r");
    if (!fifo) {
      if (errno != EINTR) perror("fopen");
      continue;
    }
