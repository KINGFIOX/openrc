  ...
  while ((size_t)(p - buffer) < bytes) {
    switch (*p) {
      ...
      case '\n':
        ...
        break;
      case '[':
        if (in_escape) in_term = true;
        break;
    }

    if (!in_escape) {
      if (!isprint((int)*p) && *p != '\n') goto cont;
      if (write(logfd, p++, 1) == -1) eerror("write: %s", strerror(errno));
      continue;
    }

    if (!in_term || isalpha((unsigned char)*p)) in_escape = in_term = false;
  cont:
    p++;
  }
}
...
void rc_logger_close(void) {
  ...
  if (signal_pipe[1] > -1) {
    if (write(signal_pipe[1], &sig, sizeof(sig)) == -1) eerror("write: %s", strerror(errno));
    close(signal_pipe[1]);
    signal_pipe[1] = -1;
  }

  if (rc_logger_pid > 0) waitpid(rc_logger_pid, 0, 0);

  if (fd_stdout > -1) dup2(fd_stdout, STDOUT_FILENO);
  if (fd_stderr > -1) dup2(fd_stderr, STDERR_FILENO);
}
...
void rc_logger_open(const char *level) {
  ...
  if (!rc_conf_yesno("rc_logger")) return;

  if (pipe(signal_pipe) == -1) eerrorx("pipe: %s", strerror(errno));
  for (i = 0; i < 2; i++)
    if ((s = fcntl(signal_pipe[i], F_GETFD, 0) == -1 || fcntl(signal_pipe[i], F_SETFD, s | FD_CLOEXEC) == -1)) eerrorx("fcntl: %s", strerror(errno));

  if (isatty(STDOUT_FILENO)) {
    tcgetattr(STDOUT_FILENO, &tt);
    ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws);
    if (openpty(&rc_logger_tty, &slave_tty, NULL, &tt, &ws)) return;
  } else if (openpty(&rc_logger_tty, &slave_tty, NULL, NULL, NULL))
    return;

  if ((s = fcntl(rc_logger_tty, F_GETFD, 0)) == 0) fcntl(rc_logger_tty, F_SETFD, s | FD_CLOEXEC);

  if ((s = fcntl(slave_tty, F_GETFD, 0)) == 0) fcntl(slave_tty, F_SETFD, s | FD_CLOEXEC);
  ...
  switch (rc_logger_pid) {
    ...
    case 0:
      ...
      if (rc_logger_tty >= 0) fd[1].fd = rc_logger_tty;
      for (;;) {
        ...
        if (fd[1].revents & (POLLIN | POLLHUP)) {
          ...
          if (write(STDOUT_FILENO, buffer, bytes) == -1) eerror("write: %s", strerror(errno));
          ...
          if (log)
            ...
            if (logbuf_size - logbuf_len < bytes) {
              logbuf_size += BUFSIZ * 10;
              logbuf = xrealloc(logbuf, sizeof(char) * logbuf_size);
            }
        ...
        if (fd[0].revents & (POLLIN | POLLHUP)) break;
      }
      if (logbuf) {
        if ((log = fopen(tmplog, "ae"))) {
          write_time(log, "started");
      ...
      if (logfile == NULL) logfile = DEFAULTLOG;
    ...
    default:
      ...
      if ((s = fcntl(fd_stdout, F_GETFD, 0)) == 0) fcntl(fd_stdout, F_SETFD, s | FD_CLOEXEC);

      if ((s = fcntl(fd_stderr, F_GETFD, 0)) == 0) fcntl(fd_stderr, F_SETFD, s | FD_CLOEXEC);
      dup2(slave_tty, STDOUT_FILENO);
      dup2(slave_tty, STDERR_FILENO);
      if (slave_tty != STDIN_FILENO && slave_tty != STDOUT_FILENO && slave_tty != STDERR_FILENO) close(slave_tty);
      close(signal_pipe[0]);
      signal_pipe[0] = -1;
      break;
  }
