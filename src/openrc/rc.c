...
const struct option longopts[] = {{"no-stop", 0, NULL, 'n'}, {"override", 1, NULL, 'o'}, {"service", 1, NULL, 's'}, {"sys", 0, NULL, 'S'}, longopts_COMMON};
const char *const longopts_help[] = {"do not stop any services", "override the next runlevel to change into\nwhen leaving single user or boot runlevels",
...
static void clean_failed(void) {
  ...
  if ((dp = opendir(failed_dir))) {
    while ((d = readdir(dp))) {
      ...
      if (unlink(path)) eerror("%s: unlink `%s': %s", applet, path, strerror(errno));
      free(path);
    }
    closedir(dp);
  }
...
static void cleanup(void) {
  ...
  if (!rc_in_logger && !rc_in_plugin && applet && (strcmp(applet, "rc") == 0 || strcmp(applet, "openrc") == 0)) {
    if (hook_out) rc_plugin_run(hook_out, runlevel);
...
static char read_key(bool block) {
  ...
  if (read(fd, &c, 1) == -1) eerror("read: %s", strerror(errno));
...
static bool want_interactive(void) {
  ...
  if (rc_yesno(getenv("EINFO_QUIET"))) return false;
  if (!gotinteractive) {
    gotinteractive = true;
...
static void run_program(const char *prog) {
  ...
  if (pid == -1) eerrorx("%s: fork: %s", applet, strerror(errno));
  if (pid == 0) {
    ...
    if (termios_orig) tcsetattr(STDIN_FILENO, TCSANOW, termios_orig);

    execl(prog, prog, (char *)NULL);
    eerror("%s: unable to exec `%s': %s", applet, prog, strerror(errno));
    _exit(EXIT_FAILURE);
  }

  /* Unmask signals and wait for child */
  sigprocmask(SIG_SETMASK, &old, NULL);
  if (rc_waitpid(pid) == -1) eerrorx("%s: failed to exec `%s'", applet, prog);
}
...
static void open_shell(void) {
  ...
  if (sys && strcmp(sys, RC_SYS_VSERVER) == 0) {
    ...
    eerrorx("%s: unable to exec `halt -f': %s", applet, strerror(errno));
  }
#endif
...
static bool set_krunlevel(const char *level) {
  ...
  if (!level || strcmp(level, getenv("RC_BOOTLEVEL")) == 0 || strcmp(level, RC_LEVEL_SINGLE) == 0 || strcmp(level, RC_LEVEL_SYSINIT) == 0) {
    if (exists(RC_KRUNLEVEL) && unlink(RC_KRUNLEVEL) != 0) eerror("unlink `%s': %s", RC_KRUNLEVEL, strerror(errno));
    return false;
  }
...
static char *get_krunlevel(void) {
  ...
  if (!exists(RC_KRUNLEVEL)) return NULL;
...
static void remove_pid(pid_t pid, bool inside_signal) {
  sigset_t sset, old;
  RC_PID *p, *tmp;
  ...
  switch (sig) {
    case SIGCHLD:
      do {
        ...
        if (pid < 0) {
          if (errno != ECHILD) eerror("waitpid: %s", strerror(errno));
          return;
        }
      } while (!WIFEXITED(status) && !WIFSIGNALED(status));

      /* Remove that pid from our list */
      if (pid > 0) remove_pid(pid, true);
      break;
...
static void do_sysinit(void) {
  ...
  if (exists(INITEARLYSH)) run_program(INITEARLYSH);

  uname(&uts);
  printf("\n   %sOpenRC %s" VERSION "%s is starting up %s", ecolor(ECOLOR_GOOD), ecolor(ECOLOR_HILITE), ecolor(ECOLOR_NORMAL), ecolor(ECOLOR_BRACKET));
#ifdef BRANDING
   ...
   * sys */
  if ((sys = rc_sys())) setenv("RC_SYS", sys, 1);
  /* force an update of the dependency tree */
  if ((main_deptree = _rc_deptree_load(1, NULL)) == NULL) eerrorx("failed to load deptree");
}
...
static void do_stop_services(RC_STRINGLIST *types_nw, RC_STRINGLIST *start_services, const RC_STRINGLIST *stop_services, const RC_DEPTREE *deptree,
  ...
  TAILQ_FOREACH_REVERSE(service, stop_services, rc_stringlist, entries) {
    ...
    if (state & RC_SERVICE_STOPPED || state & RC_SERVICE_FAILED) continue;
    ...
    if (rc_stringlist_find(kwords, "-stop") || rc_stringlist_find(kwords, "nostop") ||
        (going_down && (rc_stringlist_find(kwords, "-shutdown") || rc_stringlist_find(kwords, "noshutdown"))))
      nstop = true;
    else
      nstop = false;
    ...
    /* If the service has crashed, skip further checks and just stop
       it */
    if (crashed && rc_service_daemons_crashed(service->value)) goto stop;
    ...
    if (svc1) {
      if (newlevel && strcmp(runlevel, newlevel) != 0) {
        ...
        if (!runlevel_config(service->value, runlevel) && !runlevel_config(service->value, newlevel)) continue;
      } else
        continue;
    }
    ...
    if (!svc1) {
      ...
      deporder = rc_deptree_depends(deptree, types_nw, tmplist, newlevel ? newlevel : runlevel, RC_DEP_STRICT | RC_DEP_TRACE);
  ...
  if (types_nw_save) rc_stringlist_free(types_nw_save);

  rc_stringlist_free(nostop);
}

static void do_start_services(const RC_STRINGLIST *start_services, bool parallel) {
  RC_STRING *service;
  pid_t pid;
  ...
  if (!rc_yesno(getenv("EINFO_QUIET"))) interactive = exists(interactive_dir);
  errno = 0;
  crashed = rc_conf_yesno("rc_crashed_start");
  if (errno == ENOENT) crashed = true;

  TAILQ_FOREACH(service, start_services, entries) {
    state = rc_service_state(service->value);
    if (state & RC_SERVICE_FAILED) continue;
    if (!(state & RC_SERVICE_STOPPED)) {
      if (crashed && rc_service_daemons_crashed(service->value))
        rc_service_mark(service->value, RC_SERVICE_STOPPED);
      else
        continue;
    }
    if (!interactive) interactive = want_interactive();
  ...
  if (interactive && (strcmp(runlevel, RC_LEVEL_SYSINIT) == 0 || strcmp(runlevel, getenv("RC_BOOTLEVEL")) == 0))
    mark_interactive();
  else if (exists(interactive_dir))
    unlink(interactive_dir);
  free(interactive_dir);
}

#ifdef RC_DEBUG
RC_NORETURN static void handle_bad_signal(int sig) {
  char pid[10];
  int status;
...
int main(int argc, char **argv) {
  ...
  atexit(cleanup);
  if (!applet) eerrorx("arguments required");
  ...
  if (newlevel && (strcmp(newlevel, RC_LEVEL_SHUTDOWN) == 0 || strcmp(newlevel, RC_LEVEL_SINGLE) == 0)) {
    going_down = true;
    if (!exists(RC_KRUNLEVEL)) set_krunlevel(runlevel);
    rc_runlevel_set(newlevel);
    setenv("RC_RUNLEVEL", newlevel, 1);
    setenv("RC_GOINGDOWN", "YES", 1);
  } else {
    /* We should not use krunlevel in sysinit or boot runlevels */
    if (!newlevel || (strcmp(newlevel, RC_LEVEL_SYSINIT) != 0 && strcmp(newlevel, getenv("RC_BOOTLEVEL")) != 0)) {
    ...
    if (newlevel) {
      if (strcmp(runlevel, newlevel) != 0 && !rc_runlevel_exists(newlevel)) eerrorx("%s: not a valid runlevel", newlevel);

#ifdef __linux__
      if (strcmp(newlevel, RC_LEVEL_SYSINIT) == 0) {
        /* If we requested a runlevel, save it now */
        p = rc_proc_getent("rc_runlevel");
        if (p == NULL) p = rc_proc_getent("softlevel");
        if (p != NULL) {
          set_krunlevel(p);
          free(p);
        }
  ...
  if (newlevel && strcmp(runlevel, newlevel) != 0 && !going_down) {
    if (!rc_runlevel_exists(newlevel)) eerrorx("%s: is not a valid runlevel", newlevel);
  }

  /* Load our deptree */
  if ((main_deptree = _rc_deptree_load(0, &regen)) == NULL) eerrorx("failed to load deptree");

  xasprintf(&deptree_skewed, "%s/clock-skewed", svcdir);
  if (exists(deptree_skewed)) ewarn("WARNING: clock skew detected!");
  free(deptree_skewed);
  ...
  TAILQ_CONCAT(main_stop_services, tmplist, entries);
  free(tmplist);
  if (main_stop_services) rc_stringlist_sort(&main_stop_services);
  ...
  if (main_stop_services) {
    tmplist = rc_deptree_depends(main_deptree, main_types_nwua, main_stop_services, runlevel, depoptions | RC_DEP_STOP);
    rc_stringlist_free(main_stop_services);
  ...
  main_start_services = rc_services_in_runlevel_stacked(newlevel ? newlevel : runlevel);
  if (strcmp(newlevel ? newlevel : runlevel, RC_LEVEL_SHUTDOWN) != 0 && strcmp(newlevel ? newlevel : runlevel, RC_LEVEL_SYSINIT) != 0) {
    ...
    if (strcmp(newlevel ? newlevel : runlevel, RC_LEVEL_SINGLE) != 0) {
      ...
      if (main_hotplugged_services) {
        ...
        rc_stringlist_addu(main_start_services, service->value);
      }
    }
  }
  ...
  if (main_stop_services && !nostop) do_stop_services(main_types_nw, main_start_services, main_stop_services, main_deptree, newlevel, parallel, going_down);
   ...
   * will mount / readonly. */
  if (strcmp(runlevel, RC_LEVEL_SHUTDOWN) == 0) rc_logger_close();
#endif
  ...
  if (main_hotplugged_services) TAILQ_FOREACH(service, main_hotplugged_services, entries)
