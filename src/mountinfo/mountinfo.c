...
#elif defined(__linux__) || (defined(__FreeBSD_kernel__) && defined(__GLIBC__)) || defined(__GNU__)
#include <mntent.h>
#endif
...
const struct option longopts[] = {{"fstype-regex", 1, NULL, 'f'},  {"skip-fstype-regex", 1, NULL, 'F'},
...
static int process_mount(RC_STRINGLIST *list, struct args *args, char *from, char *to, char *fstype, char *options, int netdev) {
  char *p;
  RC_STRING *s;
  ...
  if (strcmp(fstype, "rootfs") == 0) return -1;
#endif

  if (args->netdev == net_yes && (netdev != -1 || TAILQ_FIRST(args->mounts))) {
    if (netdev != 0) return 1;
  } else if (args->netdev == net_no && (netdev != -1 || TAILQ_FIRST(args->mounts))) {
    if (netdev != 1) return 1;
  } else {
    if (args->node_regex && regexec(args->node_regex, from, 0, NULL, 0) != 0) return 1;
    if (args->skip_node_regex && regexec(args->skip_node_regex, from, 0, NULL, 0) == 0) return 1;

    if (args->fstype_regex && regexec(args->fstype_regex, fstype, 0, NULL, 0) != 0) return -1;
    if (args->skip_fstype_regex && regexec(args->skip_fstype_regex, fstype, 0, NULL, 0) == 0) return -1;

    if (args->options_regex && regexec(args->options_regex, options, 0, NULL, 0) != 0) return -1;
    if (args->skip_options_regex && regexec(args->skip_options_regex, options, 0, NULL, 0) == 0) return -1;
  }

  if (TAILQ_FIRST(args->mounts)) {
    TAILQ_FOREACH(s, args->mounts, entries)
    if (strcmp(s->value, to) == 0) break;
    if (!s) return -1;
  }
...
static RC_STRINGLIST *find_mounts(struct args *args) {
  ...
  if ((nmnts = getmntinfo(&mnts, MNT_NOWAIT)) == 0) eerrorx("getmntinfo: %s", strerror(errno));
  ...
  for (i = 0; i < nmnts; i++) {
    ...
    process_mount(list, args, mnts[i].f_mntfromname, mnts[i].f_mntonname, mnts[i].f_fstypename, options, netdev);
...
static struct mntent *getmntfile(const char *file) {
  ...
  if (!exists("/etc/fstab")) return NULL;

  fp = setmntent("/etc/fstab", "r");
  while ((ent = getmntent(fp)))
    if (strcmp(file, ent->mnt_dir) == 0) break;
  endmntent(fp);
...
static RC_STRINGLIST *find_mounts(struct args *args) {
  ...
  if ((fp = fopen(procmounts, "r")) == NULL) eerrorx("getmntinfo: %s", strerror(errno));
...
#define REG_FREE(_var) \
  ...
  while (optind < argc) {
    if (argv[optind][0] != '/') eerrorx("%s: `%s' is not a mount point", argv[0], argv[optind]);
    this_path = argv[optind++];
    real_path = realpath(this_path, NULL);
    if (real_path) this_path = real_path;
  ...
  TAILQ_FOREACH_REVERSE(s, nodes, rc_stringlist, entries) {
    if (point_regex && regexec(point_regex, s->value, 0, NULL, 0) != 0) continue;
    if (skip_point_regex && regexec(skip_point_regex, s->value, 0, NULL, 0) == 0) continue;
    if (!rc_yesno(getenv("EINFO_QUIET"))) printf("%s\n", s->value);
    result = EXIT_SUCCESS;
  }
