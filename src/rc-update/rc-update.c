...
const struct option longopts[] = {{"all", 0, NULL, 'a'}, {"stack", 0, NULL, 's'}, {"update", 0, NULL, 'u'}, longopts_COMMON};
const char *const longopts_help[] = {"Process all runlevels", "Stack a runlevel instead of a service", "Force an update of the dependency tree",
...
static int delete(const char *runlevel, const char *service) {
  ...
  if (rc_service_delete(runlevel, service)) {
    einfo("service %s deleted from runlevel %s", service, runlevel);
    return 1;
  }
...
static int addstack(const char *runlevel, const char *stack) {
  ...
  if (strcmp(runlevel, RC_LEVEL_SYSINIT) == 0 || strcmp(stack, RC_LEVEL_SYSINIT) == 0 || strcmp(runlevel, RC_LEVEL_BOOT) == 0 ||
      strcmp(stack, RC_LEVEL_BOOT) == 0 || strcmp(runlevel, RC_LEVEL_SINGLE) == 0 || strcmp(stack, RC_LEVEL_SINGLE) == 0 ||
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, getoptstring, longopts, (int *)0)) != -1) switch (opt) {
      ...
      case 'u':
        ...
        if (deptree) rc_deptree_free(deptree);
        return ret;
  ...
  if ((action & DOSHOW && action != DOSHOW) || (action & DOADD && action != DOADD) || (action & DODELETE && action != DODELETE))
  ...
  if (!action) action = DOSHOW;

  runlevels = rc_stringlist_new();

  if (optind >= argc) {
    if (!(action & DOSHOW)) eerrorx("%s: no service specified", applet);
  } else {
    service = argv[optind];
    optind++;
  ...
  retval = EXIT_SUCCESS;
  if (action & DOSHOW) {
    if (service) rc_stringlist_add(runlevels, service);
    ...
    if (!service)
      ...
      TAILQ_FOREACH(runlevel, runlevels, entries) {
        if (!rc_runlevel_exists(runlevel->value)) {
          eerror("%s: runlevel `%s' does not exist", applet, runlevel->value);
          continue;
        }
