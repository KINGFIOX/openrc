...
static const rc_service_state_name_t rc_service_state_names[] = {{RC_SERVICE_STARTED, "started"},
  ...
  while (((d = readdir(dp)) != NULL)) {
    if (d->d_name[0] != '.') {  // skip <dotfiles> <.> <..>
      if (options & LS_INITD) {
        ...
        l = strlen(d->d_name);
        if (l > 2 && d->d_name[l - 3] == '.' && d->d_name[l - 2] == 's' && d->d_name[l - 1] == 'h') continue;
      }
      if (options & LS_DIR) {
        xasprintf(&file, "%s/%s", dir, d->d_name);
        r = stat(file, &buf);
        free(file);
        if (r != 0 || !S_ISDIR(buf.st_mode)) continue;
      }
...
static bool rm_dir(const char *pathname, bool top) {
  ...
  if ((dp = opendir(pathname)) == NULL) return false;
...
static const char *detect_container(const char *systype RC_UNUSED) {
#ifdef __FreeBSD__
  if (systype) {
    if (strcmp(systype, RC_SYS_NONE) == 0) return NULL;
    if (strcmp(systype, RC_SYS_JAIL) == 0) return RC_SYS_JAIL;
    if (strcmp(systype, RC_SYS_PODMAN) == 0) return RC_SYS_PODMAN;
  }
  ...
  if (sysctlbyname("security.jail.jailed", &jailed, &len, NULL, 0) == 0)
    if (jailed == 1) return RC_SYS_JAIL;

  if (exists("/var/run/.containerenv")) return RC_SYS_PODMAN;
#endif

#ifdef __linux__
  if (systype) {
    if (strcmp(systype, RC_SYS_NONE) == 0) return NULL;
    if (strcmp(systype, RC_SYS_UML) == 0) return RC_SYS_UML;
    if (strcmp(systype, RC_SYS_VSERVER) == 0) return RC_SYS_VSERVER;
    if (strcmp(systype, RC_SYS_OPENVZ) == 0) return RC_SYS_OPENVZ;
    if (strcmp(systype, RC_SYS_LXC) == 0) return RC_SYS_LXC;
    if (strcmp(systype, RC_SYS_RKT) == 0) return RC_SYS_RKT;
    if (strcmp(systype, RC_SYS_SYSTEMD_NSPAWN) == 0) return RC_SYS_SYSTEMD_NSPAWN;
    if (strcmp(systype, RC_SYS_DOCKER) == 0) return RC_SYS_DOCKER;
    if (strcmp(systype, RC_SYS_PODMAN) == 0) return RC_SYS_PODMAN;
  }
...
static const char *detect_vm(const char *systype RC_UNUSED) {
#ifdef __NetBSD__
  if (systype) {
    if (strcmp(systype, RC_SYS_NONE) == 0) return NULL;
    if (strcmp(systype, RC_SYS_XEN0) == 0) return RC_SYS_XEN0;
    if (strcmp(systype, RC_SYS_XENU) == 0) return RC_SYS_XENU;
  }
  if (exists("/kern/xen/privcmd")) return RC_SYS_XEN0;
  if (exists("/kern/xen")) return RC_SYS_XENU;
#endif
  ...
  if (exists("/proc/xen")) {
    if (file_regex("/proc/xen/capabilities", "control_d")) return RC_SYS_XEN0;
    return RC_SYS_XENU;
  }
#endif
...
static const char *rc_parse_service_state(RC_SERVICE state) {
  ...
  for (i = 0; rc_service_state_names[i].name; i++) {
    if (rc_service_state_names[i].state == state) return rc_service_state_names[i].name;
  }
  return NULL;
}
 ...
 * specified runlevel. */
static void get_runlevel_chain(const char *runlevel, RC_STRINGLIST *level_list, RC_STRINGLIST *ancestor_list) {
  char *path;
  RC_STRINGLIST *dirs;
  RC_STRING *d, *parent;
   ...
   */
  if (!runlevel || !level_list || !rc_runlevel_exists(runlevel)) return;
   ...
   * end again.
   */
  if (rc_stringlist_find(level_list, runlevel)) rc_stringlist_delete(level_list, runlevel);
...
static bool svcdir_subpath_exists(const char *subdir) {
  char *path;
  bool found;
...
bool rc_runlevel_starting(void) { return svcdir_subpath_exists("rc.starting"); }

bool rc_runlevel_stopping(void) { return svcdir_subpath_exists("rc.stopping"); }

RC_STRINGLIST *rc_runlevel_list(void) { return ls_dir(rc_runleveldir(), LS_DIR); }
...
bool rc_runlevel_exists(const char *runlevel) {
  ...
  if (!runlevel || strcmp(runlevel, "") == 0 || strcmp(runlevel, ".") == 0 || strcmp(runlevel, "..") == 0) return false;
  xasprintf(&path, "%s/%s", rc_runleveldir(), runlevel);
  r = stat(path, &buf);
  free(path);
  if (r == 0 && S_ISDIR(buf.st_mode)) return true;
  return false;
}
...
bool rc_runlevel_stack(const char *dst, const char *src) {
  ...
  if (!rc_runlevel_exists(dst) || !rc_runlevel_exists(src)) return false;
  xasprintf(&s, "../%s", src);
...
const char *const *rc_scriptdirs(void) { return scriptdirs; }

const char *rc_sysconfdir(void) { return RC_SYSCONFDIR; }
...
bool rc_service_exists(const char *service) {
  ...
  /* .sh files are not init scripts */
  if (len > 2 && service[len - 3] == '.' && service[len - 2] == 's' && service[len - 1] == 'h') {
    errno = EINVAL;
    return false;
  }
...
#define OPTSTR ". '%s'; echo $extra_commands $extra_started_commands $extra_stopped_commands"
...
RC_STRINGLIST *rc_service_extra_commands(const char *service) {
  ...
  if (!(svc = rc_service_resolve(service))) return NULL;
...
bool rc_service_mark(const char *service, const RC_SERVICE state) {
  ...
  for (i = 0; rc_service_state_names[i].name; i++) {
    ...
    if ((s != skip_state && s != RC_SERVICE_STOPPED && s != RC_SERVICE_HOTPLUGGED && s != RC_SERVICE_SCHEDULED) &&
      ...
      if (exists(file)) {
        if ((state == RC_SERVICE_STARTING || state == RC_SERVICE_STOPPING) && s == RC_SERVICE_INACTIVE) {
          xasprintf(&was, "%s/%s/%s", svcdir, rc_parse_service_state(RC_SERVICE_WASINACTIVE), base);
          i = symlink(init, was);
          free(was);
          if (i == -1) {
            free(file);
...
RC_SERVICE rc_service_state(const char *service) {
  ...
  if (state & RC_SERVICE_STARTED) {
    if (rc_service_daemons_crashed(service) && errno != EACCES) state |= RC_SERVICE_CRASHED;
  }
  if (state & RC_SERVICE_STOPPED) {
    char *sched_dir;
...
bool rc_service_value_set(const char *service, const char *option, const char *value) {
  FILE *fp;
  char *file1, *file2;
...
bool rc_service_schedule_start(const char *service, const char *service_to_start) {
  char *file1, *file2;
  char *init;
  bool retval;

  /* service may be a provided service, like net */
  if (!service || !rc_service_exists(service_to_start)) return false;
...
RC_STRINGLIST *rc_services_scheduled_by(const char *service) {
  ...
  TAILQ_FOREACH(dir, dirs, entries) {
    ...
    if (exists(file)) rc_stringlist_add(list, file);
    free(file);
  }
  rc_stringlist_free(dirs);
  free(sched_dir);
