...
static bool pid_is_exec(pid_t pid, const char *exec) {
  ...
  if ((fp = fopen(buffer, "r"))) {
    ...
    if (c == '(') {
      while ((c = getc(fp)) != EOF && c == *exec) exec++;
      if (c == ')' && *exec == '\0') retval = true;
    }
    fclose(fp);
  }
  free(buffer);
...
static bool pid_is_argv(pid_t pid, const char *const *argv) {
  ...
  p = buffer;
  while (*argv) {
    if (strcmp(*argv, p) != 0) return false;
    argv++;
    p += strlen(p) + 1;
    if ((unsigned)(p - buffer) > sizeof(buffer)) return false;
  }
  return true;
}
...
RC_PIDLIST *rc_find_pids(const char *exec, const char *const *argv, uid_t uid, pid_t pid) {
  ...
  if ((procdir = opendir("/proc")) == NULL) return NULL;
  ...
  */
  ...
  if ((pp = getenv("RC_OPENRC_PID"))) {
    if (sscanf(pp, "%d", &openrc_pid) != 1) openrc_pid = 0;
  }
  ...
  */
  ...
  while ((entry = readdir(procdir)) != NULL) {
    ...
    if (exec && !pid_is_exec(p, exec)) continue;
    if (argv && !pid_is_argv(p, (const char *const *)argv)) continue;
...
RC_PIDLIST *rc_find_pids(const char *exec, const char *const *argv, uid_t uid, pid_t pid) {
  ...
  if ((kd = kvm_openfiles(_KVM_PATH, _KVM_PATH, NULL, _KVM_FLAGS, errbuf)) == NULL) {
  ...
  for (i = 0; i < processes; i++) {
    ...
    if (pid != 0 && pid != p) continue;
    if (uid != 0 && uid != _GET_KINFO_UID(kp[i])) continue;
    if (exec) {
      if (!_GET_KINFO_COMM(kp[i]) || strcmp(exec, _GET_KINFO_COMM(kp[i])) != 0) continue;
    }
    if (argv && *argv) {
...
static bool _match_daemon(const char *path, const char *file, RC_STRINGLIST *match) {
  char *line = NULL;
  size_t len = 0;
  char *ffile = NULL;
  FILE *fp;
  ...
  while (xgetline(&line, &len, fp) != -1) {
    ...
    if (!TAILQ_FIRST(match)) break;
  }
  fclose(fp);
  free(line);
  if (TAILQ_FIRST(match)) return false;
  return true;
}

static RC_STRINGLIST *_match_list(const char *exec, const char *const *argv, const char *pidfile) {
...
bool rc_service_daemon_set(const char *service, const char *exec, const char *const *argv, const char *pidfile, bool started) {
  char *dirpath = NULL;
  char *file = NULL;
  int nfiles = 0;
  ...
  if (started) {
    if (mkdir(dirpath, 0755) == 0 || errno == EEXIST) {
      ...
      if ((fp = fopen(file, "w"))) {
        fprintf(fp, "exec=");
        if (exec) fprintf(fp, "%s", exec);
        while (argv && argv[i]) {
          fprintf(fp, "\nargv_%d=%s", i, argv[i]);
          i++;
        }
        fprintf(fp, "\npidfile=");
        if (pidfile) fprintf(fp, "%s", pidfile);
        fprintf(fp, "\n");
        fclose(fp);
        retval = true;
      }
...
bool rc_service_started_daemon(const char *service, const char *exec, const char *const *argv, int indx) {
  char *dirpath = NULL;
  char *file = NULL;
  RC_STRINGLIST *match;
  bool retval = false;
