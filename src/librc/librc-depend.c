...
static RC_DEPINFO *get_depinfo(const RC_DEPTREE *deptree, const char *service) {
  ...
  if (deptree) {
    ...
    if (strcmp(di->service, service) == 0) return di;
  }
  return NULL;
}

static RC_DEPINFO *make_depinfo(RC_DEPTREE *deptree, const char *service) {
...
static RC_DEPTYPE *make_deptype(RC_DEPINFO *depinfo, const char *type) {
...
RC_DEPTREE *rc_deptree_load_file(const char *deptree_file) {
  ...
  if (!(fp = fopen(deptree_file, "r"))) return NULL;

  deptree = make_deptree();
  while (xgetline(&line, &size, fp) != -1) {
    p = line;
    e = strsep(&p, "_");
    if (!e || strcmp(e, "depinfo") != 0) continue;
    e = strsep(&p, "_");
    if (!e || sscanf(e, "%d", &i) != 1) continue;
    if (!(type = strsep(&p, "_="))) continue;
    ...
    e = get_shell_value(p);
    if (!e || *e == '\0') continue;
    if (!deptype || strcmp(deptype->type, type) != 0) deptype = make_deptype(depinfo, type);
...
static bool valid_service(const char *runlevel, const char *service, const char *type) {
  RC_SERVICE state;

  if (!runlevel || strcmp(type, "ineed") == 0 || strcmp(type, "needsme") == 0 || strcmp(type, "iwant") == 0 || strcmp(type, "wantsme") == 0) return true;

  if (rc_service_in_runlevel(service, runlevel)) return true;
  if (strcmp(runlevel, RC_LEVEL_SYSINIT) == 0) return false;
  if (strcmp(runlevel, RC_LEVEL_SHUTDOWN) == 0 && strcmp(type, "iafter") == 0) return false;
  if (strcmp(runlevel, bootlevel) != 0) {
    if (rc_service_in_runlevel(service, bootlevel)) return true;
  }

  state = rc_service_state(service);
  if (state & RC_SERVICE_HOTPLUGGED || state & RC_SERVICE_STARTED) return true;

  return false;
}

static bool get_provided1(const char *runlevel, RC_STRINGLIST *providers, RC_DEPTYPE *deptype, const char *level, bool hotplugged, RC_SERVICE state) {
  RC_STRING *service;
  RC_SERVICE st;
  bool retval = false;
  bool ok;
  ...
  TAILQ_FOREACH(service, deptype->services, entries) {
    ...
    if (level)
      ...
      ok = (st & RC_SERVICE_HOTPLUGGED && !rc_service_in_runlevel(svc, runlevel) && !rc_service_in_runlevel(svc, bootlevel));
    if (!ok) continue;
    switch (state) {
      ...
      case RC_SERVICE_STOPPING:
        ok = (st & RC_SERVICE_STARTING || st & RC_SERVICE_STOPPING || st & RC_SERVICE_INACTIVE);
        break;
      default:
        break;
    }
...
/* Work out if a service is provided by another service.
   ...
   */
static RC_STRINGLIST *get_provided(const RC_DEPINFO *depinfo, const char *runlevel, int options) {
  RC_DEPTYPE *dt;
  RC_STRINGLIST *providers = rc_stringlist_new();
  RC_STRING *service;

  dt = get_deptype(depinfo, "providedby");
  if (!dt) return providers;
  ...
  if (options & RC_DEP_STRICT || options & RC_DEP_START) {
    TAILQ_FOREACH(service, dt->services, entries)
    if (rc_service_in_runlevel(service->value, runlevel) || rc_service_in_runlevel(service->value, bootlevel) ||
        (options & RC_DEP_START && rc_service_state(service->value) & RC_SERVICE_HOTPLUGGED))
      rc_stringlist_add(providers, service->value);
    if (TAILQ_FIRST(providers)) return providers;
  }
...
#define DO                                             \
  ...
  if (bootlevel && strcmp(runlevel, bootlevel) != 0 && get_provided1(runlevel, providers, dt, bootlevel, false, RC_SERVICE_STARTED)) {
    DO
  }
  ...
  if (get_provided1(runlevel, providers, dt, runlevel, false, RC_SERVICE_STARTING)) return providers;
  if (get_provided1(runlevel, providers, dt, NULL, true, RC_SERVICE_STARTING)) return providers;
  if (bootlevel && strcmp(runlevel, bootlevel) != 0 && get_provided1(runlevel, providers, dt, bootlevel, false, RC_SERVICE_STARTING)) return providers;
  if (get_provided1(runlevel, providers, dt, NULL, false, RC_SERVICE_STARTING)) return providers;

  /* Nothing started then. OK, lets get the stopped services */
  if (get_provided1(runlevel, providers, dt, runlevel, false, RC_SERVICE_STOPPED)) return providers;
  if (get_provided1(runlevel, providers, dt, NULL, true, RC_SERVICE_STOPPED)) {
    DO
  }
  if (bootlevel && (strcmp(runlevel, bootlevel) != 0) && get_provided1(runlevel, providers, dt, bootlevel, false, RC_SERVICE_STOPPED)) return providers;

  /* Still nothing? OK, list our first provided service. */
  service = TAILQ_FIRST(dt->services);
  if (service != NULL) rc_stringlist_add(providers, service->value);

  return providers;
}

static void visit_service(const RC_DEPTREE *deptree, const RC_STRINGLIST *types, RC_STRINGLIST *sorted, RC_STRINGLIST *visited, const RC_DEPINFO *depinfo,
  ...
  if (strcmp(type->value, depinfo->service) == 0) return;
  /* Add ourselves as a visited service */
  rc_stringlist_add(visited, depinfo->service);

  TAILQ_FOREACH(type, types, entries) {
    if (!(dt = get_deptype(depinfo, type->value))) continue;

    TAILQ_FOREACH(service, dt->services, entries) {
      if (!(options & RC_DEP_TRACE) || strcmp(type->value, "iprovide") == 0) {
        rc_stringlist_add(sorted, service->value);
        continue;
      }

      if (!(di = get_depinfo(deptree, service->value))) continue;
      ...
      if (TAILQ_FIRST(provided)) {
        TAILQ_FOREACH(p, provided, entries) {
          ...
          if (di && valid_service(runlevel, di->service, type->value)) visit_service(deptree, types, sorted, visited, di, runlevel, options | RC_DEP_TRACE);
        }
      } else if (di && valid_service(runlevel, service->value, type->value))
        visit_service(deptree, types, sorted, visited, di, runlevel, options | RC_DEP_TRACE);
  ...
  if (!svcname || strcmp(svcname, depinfo->service) != 0) {
    if (!get_deptype(depinfo, "providedby")) rc_stringlist_add(sorted, depinfo->service);
  }
}

RC_STRINGLIST *rc_deptree_depend(const RC_DEPTREE *deptree, const char *service, const char *type) {
  RC_DEPINFO *di;
  RC_DEPTYPE *dt;
  RC_STRINGLIST *svcs;
  RC_STRING *svc;
  ...
  if (!(di = get_depinfo(deptree, service)) || !(dt = get_deptype(di, type))) {
    errno = ENOENT;
    return svcs;
  }
...
RC_STRINGLIST *rc_deptree_depends(const RC_DEPTREE *deptree, const RC_STRINGLIST *types, const RC_STRINGLIST *services, const char *runlevel, int options) {
  ...
  if (!bootlevel) bootlevel = RC_LEVEL_BOOT;
  TAILQ_FOREACH(service, services, entries) {
    if (!(di = get_depinfo(deptree, service->value))) {
      errno = ENOENT;
      continue;
    }
    if (types) visit_service(deptree, types, sorted, visited, di, runlevel, options);
  }
  rc_stringlist_free(visited);
  return sorted;
}

RC_STRINGLIST *rc_deptree_order(const RC_DEPTREE *deptree, const char *runlevel, int options) {
  RC_STRINGLIST *list;
  RC_STRINGLIST *list2;
  RC_STRINGLIST *types;
  ...
  if (strcmp(runlevel, RC_LEVEL_SINGLE) == 0 || strcmp(runlevel, RC_LEVEL_SHUTDOWN) == 0) {
...
*/
static bool deep_mtime_check(const char *target, bool newer, time_t *rel, char *file) {
  struct stat buf;
  bool retval = true;
  DIR *dp;
  struct dirent *d;
  ...
  if (stat(target, &buf) != 0) return true;

  if (newer) {
    if (*rel < buf.st_mtime) {
      retval = false;

      if (file) strlcpy(file, target, PATH_MAX);
      *rel = buf.st_mtime;
    }
  } else {
    if (*rel > buf.st_mtime) {
 ...
 */
static bool mtime_check(const char *source, const char *target, bool newer, time_t *rel, char *file) {
  struct stat buf;
  time_t mtime;
  bool retval = true;

  /* We have to exist */
  if (stat(source, &buf) != 0) return false;
  mtime = buf.st_mtime;
...
bool rc_newer_than(const char *source, const char *target, time_t *newest, char *file) { return mtime_check(source, target, true, newest, file); }

bool rc_older_than(const char *source, const char *target, time_t *oldest, char *file) { return mtime_check(source, target, false, oldest, file); }
...
static const DEPPAIR deppairs[] = {{"ineed", "needsme"},  {"iuse", "usesme"},         {"iwant", "wantsme"}, {"iafter", "ibefore"},
                                   {"ibefore", "iafter"}, {"iprovide", "providedby"}, {NULL, NULL}};

static const char *const depdirs[] = {"",           "starting", "started", "stopping",  "inactive",  "wasinactive", "failed",
                                      "hotplugged", "daemons",  "options", "exclusive", "scheduled", "tmp",         NULL};
...
bool rc_deptree_update_needed(time_t *newest, char *file) {
  ...
  TAILQ_FOREACH(s, config, entries) { newer |= !deep_mtime_check(s->value, true, &mtime, file); }
...
static void setup_environment(void) {
  ...
  for (const char *const *dirs = rc_scriptdirs(); *dirs; dirs++) env_size += strlen(*dirs) + sizeof(' ');
  ...
  if (!getenv("RC_LIBEXECDIR")) setenv("RC_LIBEXECDIR", RC_LIBEXECDIR, 0);

  if (uname(&uts) == 0) setenv("RC_UNAME", uts.sysname, 1);
}
...
bool rc_deptree_update(void) {
  ...
  setup_environment();
  if (!(fp = popen(GENDEP, "r"))) return false;
  ...
  while (xgetline(&line, &size, fp) != -1) {
    ...
    if (!depinfo || strcmp(depinfo->service, service) != 0) {
      ...
      if (!depinfo) depinfo = make_depinfo(deptree, service);
    }
    ...
    while ((depend = strsep(&depends, " "))) {
      ...
      if (strcmp(depend, service) == 0) continue;

      /* .sh files are not init scripts */
      l = strlen(depend);
      if (l > 2 && depend[l - 3] == '.' && depend[l - 2] == 's' && depend[l - 1] == 'h') continue;
      ...
      if (strcmp(type, "ibefore") == 0) {
        if ((dt = get_deptype(depinfo, "iafter"))) rc_stringlist_delete(dt->services, depend);
      }
      /* If we're after something, remove us from the before list */
      if (strcmp(type, "iafter") == 0 || strcmp(type, "ineed") == 0 || strcmp(type, "iwant") == 0 || strcmp(type, "iuse") == 0) {
        if ((dt = get_deptype(depinfo, "ibefore"))) rc_stringlist_delete(dt->services, depend);
      }
    }
  }
  free(line);
  ...
  if (sys) {
    ...
    nosys = xmalloc(len + 2);
    nosys[0] = '-';
    for (i = 0; i < len; i++) nosys[i + 1] = (char)tolower((unsigned char)sys[i]);
    nosys[i + 1] = '\0';

    onosys = xmalloc(len + 3);
    onosys[0] = 'n';
    onosys[1] = 'o';
    for (i = 0; i < len; i++) onosys[i + 2] = (char)tolower((unsigned char)sys[i]);
    onosys[i + 2] = '\0';

    TAILQ_FOREACH_SAFE(depinfo, deptree, entries, depinfo_np) {
      if (!(deptype = get_deptype(depinfo, "keyword"))) continue;
      TAILQ_FOREACH(s, deptype->services, entries) {
        if (strcmp(s->value, nosys) != 0 && strcmp(s->value, onosys) != 0) continue;
        ...
        TAILQ_FOREACH(di, deptree, entries) {
          TAILQ_FOREACH_SAFE(dt, &di->depends, entries, dt_np) {
            ...
            if (provide) TAILQ_FOREACH(s2, provide->services, entries)
  ...
  TAILQ_FOREACH(depinfo, deptree, entries) {
    if (!(deptype = get_deptype(depinfo, "iprovide"))) continue;
    TAILQ_FOREACH(s, deptype->services, entries) {
      di = get_depinfo(providers, s->value);
      if (!di) di = make_depinfo(providers, s->value);
    }
  }
      ...
      TAILQ_FOREACH(s, deptype->services, entries) {
        ...
        if (!di) {
          if (strcmp(deptype->type, "ineed") == 0) {
            ...
            if (!dt) dt = make_deptype(depinfo, "broken");
        ...
        if (!dt) dt = make_deptype(di, deppairs[i].addto);
  ...
  TAILQ_FOREACH(depinfo, deptree, entries) {
    ...
    visit_service(deptree, types, sorted, visited, depinfo, NULL, 0);
    rc_stringlist_free(visited);
    TAILQ_FOREACH_SAFE(s2, deptype->services, entries, s2_np) {
      TAILQ_FOREACH(s3, sorted, entries) {
        ...
        if (strcmp(s2->value, s3->value) == 0) {
          ...
          if (dt) rc_stringlist_delete(dt->services, depinfo->service);
          break;
        }
        dt = get_deptype(di, "iprovide");
        if (!dt) continue;
        TAILQ_FOREACH(s4, dt->services, entries) {
          if (strcmp(s4->value, s2->value) == 0) break;
        }
        if (s4) {
      ...
      if (s3) rc_stringlist_delete(deptype->services, s2->value);
    }
  ...
  TAILQ_FOREACH(depinfo, deptree, entries) {
    ...
    if (errno == EEXIST) {
      fprintf(stderr, "Error: %s is the name of a real and virtual service.\n", depinfo->service);
    }
    errno = serrno;
  }
  ...
  if ((fp = fopen(deptree_cache, "w"))) {
    ...
    TAILQ_FOREACH(depinfo, deptree, entries) {
      ...
      TAILQ_FOREACH(deptype, &depinfo->depends, entries) {
        size_t k = 0;
        TAILQ_FOREACH(s, deptype->services, entries) { fprintf(fp, "depinfo_%zu_%s_%zu='%s'\n", i, deptype->type, k++, s->value); }
      }
      i++;
    }
    fclose(fp);
