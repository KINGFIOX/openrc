...
const struct option longopts[] = {{"seed-dir", 1, NULL, LONGOPT_SEED_DIR}, {"skip-credit", 0, NULL, LONGOPT_SKIP_CREDIT}, longopts_COMMON};
const char *const longopts_help[] = {"Directory for seed files (default: " DEFAULT_SEED_DIR ")", "Skip crediting entropy of seeds", longopts_help_COMMON};
...
static inline uint32_t ror32(uint32_t word, unsigned int shift) { return (word >> (shift & 31)) | (word << ((-shift) & 31)); }

static const uint32_t blake2s_iv[8] = {0x6A09E667UL, 0xBB67AE85UL, 0x3C6EF372UL, 0xA54FF53AUL, 0x510E527FUL, 0x9B05688CUL, 0x1F83D9ABUL, 0x5BE0CD19UL};
...
static void blake2s_set_lastblock(struct blake2s_state *state) { state->f[0] = -1; }
...
static void blake2s_init_param(struct blake2s_state *state, const uint32_t param) {
  ...
  for (i = 0; i < 8; ++i) state->h[i] = blake2s_iv[i];
  state->h[0] ^= param;
}
...
#undef ROUND
    ...
    for (i = 0; i < 8; ++i) state->h[i] ^= v[i] ^ v[i + 8];
...
static size_t determine_optimal_seed_len(void) {
  ...
  if (fd < 0 || read_full(fd, poolsize_str, sizeof(poolsize_str) - 1) < 0) {
    ewarn("%s: Unable to determine pool size, falling back to %u bits: %s", applet, MIN_SEED_LEN * 8, strerror(errno));
    ret = MIN_SEED_LEN;
  } else
  ...
  *is_creditable = false;
  ...
  if (ret == (ssize_t)len) {
    *is_creditable = true;
    ...
    if (random_fd.fd < 0) return -errno;
...
static int seed_from_file_if_exists(const char *filename, int dfd, bool credit, struct blake2s_state *hash) {
  ...
  if ((unlinkat(dfd, filename, 0) < 0 || fsync(dfd) < 0) && seed_len) {
    ret = -errno;
    eerror("%s: Unable to remove seed after reading, so not seeding: %s", applet, strerror(errno));
    goto out;
  }
  if (!seed_len) goto out;
...
int main(int argc, char **argv) {
  ...
  if (!seed_dir) seed_dir = DEFAULT_SEED_DIR;
  if (getuid()) eerrorx("%s: superuser access is required", applet);
  umask(0077);
  ...
  if (mkdir(seed_dir, 0700) < 0 && errno != EEXIST) eerrorx("%s: Unable to create seed directory: %s", applet, strerror(errno));

  dfd = open(seed_dir, O_DIRECTORY | O_RDONLY);
  if (dfd < 0 || flock(dfd, LOCK_EX) < 0) eerrorx("%s: Unable to lock seed directory: %s", applet, strerror(errno));

  if (seed_from_file_if_exists(NON_CREDITABLE_SEED, dfd, false, &hash) < 0) program_ret |= 1 << 1;
  if (seed_from_file_if_exists(CREDITABLE_SEED, dfd, !skip_credit, &hash) < 0) program_ret |= 1 << 2;
