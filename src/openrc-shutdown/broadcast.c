...
static int file_isatty(const char *fname) {
  ...
  if (major == 1 || major == 2 || major == 6 || major == 9 || major == 12 || major == 16 || major == 21 || major == 27 || major == 37 || major == 96 ||
      major == 97 || major == 206 || major == 230)
    return 0;
  return 1;
}
...
void broadcast(char *text) {
  ...
  len = xasprintf(&line, "\007\r\nBroadcast message from %s@%s %s(%s):\r\n\r\n%s", user, name.nodename, tty, date, text);
  free(tty);
  ...
  while ((utmp = getutxent()) != NULL) {
    if (utmp->ut_type != USER_PROCESS || utmp->ut_user[0] == 0) continue;
    ...
    if (file_isatty(term) && (fd = open(term, O_WRONLY | O_NDELAY | O_NOCTTY)) >= 0) {
      if (isatty(fd)) write(fd, line, len);
      close(fd);
    }
    free(term);
  }
