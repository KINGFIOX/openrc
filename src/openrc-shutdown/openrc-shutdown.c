...
static void cancel_shutdown(void) {
  ...
  if (pid <= 0) eerrorx("%s: Unable to cancel shutdown", applet);
...
static void send_cmd(const char *cmd) {
  ...
  if (do_wtmp && (do_halt || do_kexec || do_reboot || do_poweroff)) log_wtmp("shutdown", "~~", 0, RUN_LVL, "~~");
  ...
  if (ignored != strlen(cmd)) printf("Error writing to init fifo\n");
  fclose(fifo);
}
...
static void sleep_no_interrupt(int seconds) {
  ...
  while (nanosleep(&duration, &remaining) < 0 && errno == EINTR) duration = remaining;
}
...
int main(int argc, char **argv) {
  ...
  if (geteuid() != 0) eerrorx("%s: you must be root\n", applet);
  if (cmd_count != 1) {
    eerror("%s: %s\n", applet, exclusive);
    usage(EXIT_FAILURE);
  }
  ...
  if (strcasecmp(time_arg, "now") == 0) strcpy(time_arg, "0");
  ...
  if (strchr(time_arg, ':')) {
    if ((sscanf(time_arg, "%2d:%2d", &hour, &min) != 2) || (hour > 23) || (min > 59)) {
      eerror("%s: invalid time %s", applet, time_arg);
      usage(EXIT_FAILURE);
    }
    time(&tv);
    ...
    if (shutdown_delay < 0) shutdown_delay += 1440;
  } else {
    shutdown_delay = atoi(time_arg);
  }

  fp = fopen(shutdown_pid, "w");
  if (!fp) eerrorx("%s: fopen `%s': %s", applet, shutdown_pid, strerror(errno));
  fprintf(fp, "%d\n", getpid());
  fclose(fp);
  ...
  while (shutdown_delay > 0) {
    ...
    if (shutdown_delay <= 5) create_nologin(shutdown_delay);
    if (need_warning) {
